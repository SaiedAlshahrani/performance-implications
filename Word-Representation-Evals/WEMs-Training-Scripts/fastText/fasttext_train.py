...
if __name__ == '__main__':
    ...
    if len(sys.argv) < 5:
        print('Usage: python fasttext_train.py <wikipedia_dump_text_file> <fasttext_algorithm:0->cbow,1->skip-gram> <save_bin_model> <save_model_vectors>')
        sys.exit(1)

    wiki_corpus, fasttext_algorithm, save_bin_model, save_model_vectors = sys.argv[1:5]

    logging.basicConfig(filename=f'{save_bin_model}.log',filemode='a',format='%(asctime)s : %(levelname)s : %(message)s', level=logging.INFO)

    print(f"# Training fastText on '{wiki_corpus.split('/')[1]}' in progress!")
    ...
    model = FastText(LineSentence(wiki_corpus), vector_size=vector_size, window=window_size, min_count=min_count, sample=sampling_threshold, hs=0,
                alpha=alpha, min_alpha=min_alpha, workers=worker_count, sg=fasttext_algorithm, negative=negative_size, cbow_mean=1, epochs=train_epoch)

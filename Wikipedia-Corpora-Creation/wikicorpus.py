...
import re
import requests ####### Added by Alshahrani et al. #######
import signal
from bs4 import BeautifulSoup  ####### Added by Alshahrani et al. #######
...
####### Added by Alshahrani et al. #######
def get_bots_list(wiki_code):
    if wiki_code=='arz':
        url = 'https://arz.wikipedia.org/wiki/خاص:عرض_اليوزرات?username=&group=bot&wpsubmit=&wpFormIdentifier=mw-listusers-form&limit=1000'
    url = 'https://'+wiki_code+'.wikipedia.org/wiki/خاص:عرض_المستخدمين?username=&group=bot&wpsubmit=&wpFormIdentifier=mw-listusers-form&limit=1000'
    bots_list = re.findall("<" + "bdi" + ">(.*?)</" + "bdi" + ">", str(BeautifulSoup(requests.get(url).content, "html.parser")))
    return bots_list

####### Added by Alshahrani et al. #######
def get_page_creator(wiki_code, title):
    url = f"https://xtools.wmcloud.org/api/page/articleinfo/{wiki_code}.wikipedia.org/{title}?format=json"
    page_creator = eval(str(BeautifulSoup(requests.get(url).content, "html.parser")).replace('null', 'None'))['author']
    return page_creator
    ...
    * <LANG>wiki-latest-pages-articles.xml.bz2
    ...
    def __init__(
        ...
        if dictionary is None:
            # self.dictionary = Dictionary(self.get_texts()) ####### Added by Alshahrani et al. #######
            pass ####### Added by Alshahrani et al. #######
        else:
    ...
    def get_texts(self):
        ...
        ####### Added by Alshahrani et al. #######
        wiki_code = self.fname.split('/')[-1].split('-')[0].strip('wiki')
        bots_list = get_bots_list(wiki_code)
        bots_list.append('Unknown')
        print(f'Bots in {str(wiki_code).upper()}: {bots_list}')
        ...
        # print(texts) ####### Added by Alshahrani et al. #######
        ...
        try:
            ...
            for group in utils.chunkize(texts, chunksize=10 * self.processes, maxsize=1):
                for tokens, title, pageid in pool.imap(_process_article, group):
                    # print(title) ####### Added by Alshahrani et al. #######
                    articles_all += 1
                    ...
                    try: page_creator = get_page_creator(wiki_code, title) ####### Added by Alshahrani et al. #######
                    except KeyError: page_creator =='Unknown'
                    except SyntaxError: pass

                    ####### Added by Alshahrani et al. #######
                    if page_creator in bots_list:
                        continue

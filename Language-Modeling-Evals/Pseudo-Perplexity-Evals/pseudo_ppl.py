...
hflogging.disable_progress_bar()
...
def compute_perplexity(dataset, samples, number_samples):
    ...
    print(f"\n# Calculating Pseudo-Perplexity of `{dataset}` in Progress ... ")
    ...
    print(f"\n# Mean Pseudo-Perplexity of `{number_samples}` Randomly Selected Samples from `{dataset}` ==", '{0:,.2f}'.format(ppl_mean))
